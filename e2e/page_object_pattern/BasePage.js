...
class BasePage {
   ...
   * @param {string} url The url of the page which gets passed from the inheriting page
   */
  constructor(url) {
    this.url = url;
   ...
   * It also updates the current property of the passed in page object plus it updates the
   * currentDomainUrl which is available to all pages that inherit from this class.
  ...
  testCafeNavigateToPage(page) {
    ...
    return prmNav.then(() => this.pageLoaded().then((isPageLoaded) => {
      if (isPageLoaded) {
        page.current = this;
      }
      return isPageLoaded;
