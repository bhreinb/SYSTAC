...
1) Add the page to the page object map which can be found here @ `./e2e/config/Json/PageObjectMap.json` (note the url specified here gets added to `BaseUrls.json` file automatically by the framework thus allowing the page object to work across multiple environment types).
2) Create the page object within folder location `./e2e/page_object_pattern` and export it (doing that means it gets auto picked up by the framework whenever a test run is initiated).
